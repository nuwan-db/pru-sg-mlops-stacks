...
The databricks CLI bundles top level is defined by file `pru_sg_mlops_stacks/bundle.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`pru_sg_mlops_stacks/databricks-resources/model-workflow-resource.yml`)
 - batch inference workflow (`pru_sg_mlops_stacks/databricks-resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`pru_sg_mlops_stacks/databricks-resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`pru_sg_mlops_stacks/databricks-resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`pru_sg_mlops_stacks/databricks-resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` environment is used by ML engineers to deploy ML resources to development workspace with `dev` environment configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-pru-sg-mlops-stacks-model     | /dev-pru-sg-mlops-stacks-experiment     |
| staging     | The `staging` environment is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` environment config.                                                             | staging workspace    | staging-pru-sg-mlops-stacks-model | /staging-pru-sg-mlops-stacks-experiment |
| prod        | The `prod` environment is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` environment config.                                                                      | prod workspace       | prod-pru-sg-mlops-stacks-model    | /prod-pru-sg-mlops-stacks-experiment    |
| test        | The `test` environment is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` environment. | staging workspace    | test-pru-sg-mlops-stacks-model    | /test-pru-sg-mlops-stacks-experiment    |
...
2. Complete the `TODO` in `pru_sg_mlops_stacks/bundle.yml` to add the dev workspace URI under `environments.dev.workspace.host`.
...
4. Set an env variable `DATABRICKS_TOKEN` with your Databricks personal access token in your terminal. 
...
Following this, update the batch_inference_job base parameters in `pru_sg_mlops_stacks/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `pru_sg_mlops_stacks/databricks-resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.environment}-pru-sg-mlops-stacks-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.environment}-pru-sg-mlops-stacks-batch-inference-job`
that runs the notebook under `pru_sg_mlops_stacks/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.environment}-pru-sg-mlops-stacks-batch-inference-job`. 
...
`test-pru-sg-mlops-stacks-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-pru-sg-mlops-stacks-batch-inference-job`.
